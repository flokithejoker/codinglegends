...
class BaseAgent:
    ...
    def __init__(self, provider: str, model: str, prompt_name: str, temperature: float = 0.0, max_tokens: int = 4096,):
        self.provider = provider
        self.model_name = model
        self.prompt_name = prompt_name
        self.temperature = temperature
        self.max_tokens = max_tokens
        self.llm = get_model(provider, model, temperature=temperature, max_tokens=max_tokens)
    ...
    def run_batch(self, inputs: list[dict[str, Any]], max_concurrency: int = 4):
        ...
        return self._structured_llm.batch(all_messages, config={"max_concurrency": max_concurrency})